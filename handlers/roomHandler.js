...
// ─────────────────────────────────────────────────────────────────────────────
// FIX — ÉTAT DU LOT ACTIF PAR ROOM
//
// Persisté via store.setLotState()/store.getLotState() (voir store.js), sur
// un Hash Redis DÉDIÉ (LOTS_HASH_KEY), séparé de celui des sockets. On
// réutilise donc la même connexion Redis déjà en place dans store.js, sans
// créer de nouvel état parallèle ni polluer le Hash des sockets.
//
...
function registerRoomHandler(io, socket) {
  ...
  socket.on("getMsgRoom", async (data) => {
    ...
    if (data.type === "numLot" && data.msg?.numLot !== undefined) {
      await store.setLotState(data.room, {
